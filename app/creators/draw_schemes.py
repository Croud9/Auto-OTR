...
def draw(data, i, gost_frame_params, config):
    ...
    with schemdraw.Drawing(file=f'Data/Schemes/Invertor/invertor_{i}.svg', show=False, scale = 0.5, lw = 0.7, font = 'sans-serif') as slr:
        ...
        slr.here = (0, 3)
        if isinstance(i, str):
            title_for_inv = f"Инверторы {i}"
            title_for_frame = f"инверторов {i}"
        else:
            title_for_inv = f"Инвертор {i}"
            title_for_frame = f"инвертора {i}"
        ...
        data = {'width': width, 'height': height, 'title_prjct': gost_frame_params['title_project'],
                'code_prjct': gost_frame_params['code_project'], 'controller': False, 'type_scheme': f"Cхема постоянного тока\nдля {title_for_frame}"}
