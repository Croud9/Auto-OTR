import os, re
import wikipedia
from views import designParsing, styles_and_animation
from helpers import parsingSelenium, validate
...
class Parsing(QThread):
    ...
    def delete_files(self):
        ...
        patch_imgs_pptx = "Data/System/Images/PPTX"
        img_files_pvsyst = [f for f in os.listdir(patch_imgs_pvsyst) if os.path.isfile(os.path.join(patch_imgs_pvsyst, f))]
        img_files_pptx = [f for f in os.listdir(patch_imgs_pptx) if os.path.isfile(os.path.join(patch_imgs_pptx, f))]
        ...
        if len(img_files_pptx) != 0:
            for file in img_files_pptx:
                os.remove(patch_imgs_pptx + f"/{file}")  
...
class WindowParse(QtWidgets.QMainWindow, designParsing.Ui_WindowRP5):
    ...
    def downloadFinished(self):
        if self.parser_download.return_download == 1:
            ...
            QTimer.singleShot(5000, lambda: self.statusBar.setStyleSheet(styles_and_animation.status_white))
        else:
