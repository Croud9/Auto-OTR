...
if __name__ == '__main__':  # Если мы запускаем файл напрямую, а не импортируем
    ...
    #  1. ТЗ (форма, заполненная заказчиком)
    # Блоки:
    #          Анализ(доп блок, добавить позже)
    #  1. Климатология (файл с точками для формирования графиков, которые вставляются в отчёт)
    #  2. Топография(фотки вставляются в отчёт)
    #  3. Расчёты и схемы(выгрузка с PVsyst, графики, сформировать базу и реализовать подгрузку информации/фоток в зависимости от модуля)
    #  4. Графики(схема автоматически изменяемая в зависимости от вводных данных(автоматическое взаимодействие с AutoCAD/Visio), генплан(готовый просто загрузить), типовая схема(подгрузить)
    #
    # Реализовать вставку этих блоков в шаблон отчета и удаление блоков по выбору
    #
    # Необходимо:
    #  1. Шаблон в который будут вставляться данные(также примерный отчёт сформированный ранее вручную)
    #  2. Входные данные разделённые по модулям:
    #          1.  Файл с точками для формирования графиков
    #          2.  Топографические снимки(фото/ любой другой формат, который должен присутствовать в итоговом отчете)
    #          3.  Файл с выгрузкой pvsyst, графики, данные по модулям для формирования базы данных модулей
    #          4.  Графики/схемы(пример схемы и в зависимости от каких файлов/данных должна изменяться схема, генплан, типовые схемы)
